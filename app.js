(function () {
  ...
  const TYPES = {
    white: {
      ...
      offer: "家計金運鑑定、老後不安を整える鑑定、生活の金運改善講座",
    ...
    green: {
      ...
      offer: "家族運鑑定、住まいの金運診断、家族を守るお金の整え方",
    ...
    moon: {
      ...
      offer: "毎日の龍神メッセージ、LINE限定音声、癒しの鑑定",
    ...
    gold: {
      ...
      offer: "個別鑑定、副業・資産形成コンテンツ、人生転機のセッション",
  ...
  function runAction(action, element) {
    ...
    if (action === "line") {
      markLineClicked();
      if (state.result) {
        sendLineHarnessTags(state.result, "line_cta_clicked");
      }
      window.open(buildLineAuthUrl(), "_blank", "noopener");
    }
    ...
    return `
      <section class="screen">
        ...
        <div class="panel result-section">
          ...
          <p class="notice">Lハーネス連携を有効化すると、LIFFで確認できたユーザーへ診断タグを送信します。</p>
        </div>

        <div class="panel line-offer result-section">
          <h2>あなたの診断結果には、まだ続きがあります</h2>
          <p>今のあなたに必要な、金運が乱れやすい場所・近いうちに起こりやすい転機・あなた専用の開運行動をまとめた無料鑑定書をLINEでお渡しします。</p>
          <ul class="mini-list">
            <li>${main.offer}</li>
            <li>診断結果をLINE内で再確認できます</li>
            <li>配信停止はLINE上でいつでも可能です</li>
          </ul>
          <div class="cta-row">
    ...
    const docs = {
      privacy: {
        ...
        body: [
          ...
          "健康状態、病歴、宗教、政治思想などのセンシティブ情報は原則として取得しません。配信停止はLINE上のブロックまたは案内された方法で行えます。"
        ]
      },
      terms: {
        title: "利用規約",
